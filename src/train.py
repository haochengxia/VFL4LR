...
def vfl_lr_train(server :Server, clients: List[Client]):
    test_loss, test_acc = eval(server, clients)
    ...
    for epoch in range(server.epoch_num):
        ...
        for i in range(batch_num):
            batch_indexs = batches[i]
            # Init clients
            for c in clients:
                c.set_batch_indexs(batch_indexs)
...
def eval(server : Server, clients: List[Client]):
    ...
    for idx, y in enumerate(server.Y_test):
        pred_proba = softmax(aggr_embedding_data[:, idx] + server.bias)
        test_loss -= np.log(pred_proba[y])
        if np.argmax(pred_proba) == y:
            test_acc += 1
